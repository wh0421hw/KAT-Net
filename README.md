...
DSSP is used to derive relative solvent accessibility (RSA) and secondary-structure assignments.
...
Compatibility note: In the structural feature files used by the released training and testing code, RSA is stored under the legacy NPZ key sasa. This historical key name is retained only for backward compatibility.
...
The generated structural information includes residue coordinates, pLDDT, PAE, DSSP-derived RSA, three-state secondary structure, and 64-bin distogram probabilities.
...
DSSP-derived RSA is used as the solvent-accessibility feature.

For backward compatibility with the released feature files and data-loading code, RSA may be stored under the legacy NPZ key sasa.
